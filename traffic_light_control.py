...
class DQNAgent:
    ...
    def _build_model(self):
        ...
        input_1 = Input(shape=(12, 12, 1))      #Positions
...
class SumoIntersection:
    ...
    <vType id="SUMO_DEFAULT_TYPE" accel="0.8" decel="4.5" sigma="0" length="5" minGap="2" maxSpeed="19.44"/>
    ...
    ''', file=routes)
            ...
            for i in range(N):
                ...
                if random.uniform(0, 1) < pAL:
                    print('    <vehicle id="down_%i" type="SUMO_DEFAULT_TYPE" route="always_left" depart="%i" color="0,0,1"/>' % (
                        vehNr, i), file=routes)
                    vehNr += 1
                    lastVeh = i
...
if __name__ == '__main__':
    ...
    agent = DQNAgent()
    try:
        models_list = os.listdir(os.path.join(os.getcwd(), 'Models'))
        agent.load("Models/" + models_list[-1])
        print('Loaded model: ' + models_list[-1])
        for i in models_list:
        	os.remove("Models/" + i)
        agent.save("Models/" + models_list[0])
    except:
    ...
    for e in range(episodes):
        ...
        agent.save('Models/reinf_traf_control_{:03}'.format(e) + '.h5')
